...
float planeHit(in vec3 norm, in vec3 po, in vec3 ro, in vec3 rd)
{
...
vec2 raytraceTexturedQuad(in vec3 rayOrigin, in vec3 rayDirection, in vec3 quadCenter, in vec3 quadRotation, in vec2 quadDimensions)
{
	...
	vec3 pos = (rayDirection * planeHit(normal, quadCenter, rayOrigin, rayDirection)) - quadCenter;
...
void main()
{
	...
	vec3 planeRotation = vec3(xrot, yrot + PI, zrot);
